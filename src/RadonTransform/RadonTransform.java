...
public class RadonTransform {
    ...
    return -57.295779513082320876798154814105 *skewRadians;
}
  ...
  static double findSkew(final BufferedImage img) 
  {
    ...
    final double iskew = imax - w2 + 1;
      //System.out.println("lesnght"+sharpness.length);
    //Histogram hist = new Histogram(sharpness);-----------------------------plotea intensidad de líneas
  ...
  static void radon(final int width, final int height, final DataBuffer buffer, final int sign,
    ...
    for (row = 0; row < h; row++) {
      ...
      for (column = 0; column < w; column++, count++) {
          color = 100;
        if (sign > 0) {
          final int b = buffer.getElem(0, scanlinePosition + w - 1 - column);
          p1_[h * column + row] = BitUtils.bitcount_[b];
          color += BitUtils.bitcount_[b];
          if(BitUtils.bitcount_[b]>0)
              color+=100;
          
          //bi.setRGB(column, row, new Color(color,color,color).getRGB());
          //bi.setRGB(column, row, color);
          //System.out.println(BitUtils.bitcount_[b]);
        } else {
    ...
    //String extension = Funciones_Image.VerificaFormato(""+".png");
                //Funciones_Image.ImagenAFormato(bi, "C:\\TEMP\\test\\E.png", "png");
      //System.out.println("péxles: "+count);
    int[] x1 = p1_;
